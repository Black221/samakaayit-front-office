...
export default function Details_v2() {
  ...
  const { fetchRequestById, requestResponse, requestLoading, requestError } =
    useRequest();
  ...
  const onResponse = () => {
    openModal(<Modal demande={requestResponse?.data} onClose={closeModal} />);
  };

  const onRefuse = () => {
    openModal(
      <RefuseModal onClose={closeModal} demande={requestResponse?.data} />
    );
  };
  ...
  const doc: {
    ...
    return (<>
        <div className="">
            <button
            className="mb-4 h-8 px-4 text-sm text-white transition-colors duration-150 bg-primary-700 rounded-lg focus:shadow-outline hover:bg-gray-800"
            type="button"
            onClick={() => {
                navigate(-1);
            }}
