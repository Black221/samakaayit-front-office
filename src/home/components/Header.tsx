...
export default function Header ({ref, top, toggle}: IHeader) {
    ...
    return (<>
        <header ref={ref} className={`fixed w-screen ${top < 0 ? "-top-[80px]" : "top-0"} z-[1000]`} style={{
            transition: "top 0.6s"
        }}> 
            <nav className="w-full bg-white border-b border-gray-200 px-4 lg:px-6 py-2.5">
...
const Menu = ({toggle}: {toggle:React.Dispatch<React.SetStateAction<boolean>>}) => {
    ...
    return(<>
        <div className="flex items-center lg:order-2">
            <Button label="Espace Membre" className="uppercase text-white bg-blue-600" />
            <button onClick={ () => {
                    toggle(prev => !prev)
...
const Logo = () => {
    ...
    return(<>
        <a href="https://flowbite.com" className="flex items-center">
            ...
            <span className="self-center text-xl font-semibold whitespace-nowrap ">Synapse</span>
        </a>
    </>)
}
